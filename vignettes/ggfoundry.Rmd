...
p2 <- p +
  geom_text(aes(label = label), colour = "black", fontface = "bold",
            size = 24, show.legend = FALSE) +
  geom_text(aes(label = label, colour = factor(x)), 
            size = 20, show.legend = FALSE) +
  geom_text(aes(label = x), colour = "black", nudge_y = -0.15,
...
After travelling the mountains, seas and forests of the world in search of that elusive shape (or set of up to 6), a hand-made grob may be the fillable "Holy Grail" sought via a Github issue.
...
list.files(system.file("extdata", package = "ggfoundry")) |>
  ...
  separate_wider_delim(value, delim = "-", names = c("set", "shapes")) |>
  distinct() |>
  summarise(shapes = str_c(shapes, collapse = ", "), .by = set) |>
  gt() |> 
  tab_options(
