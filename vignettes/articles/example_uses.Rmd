...
`display_palette()` shows the loaded palette with associated hex codes using the default shape `jar` from the ggfoundry `container` set. The outline colour defaults to mid-grey for better dark-mode support as per this [example post](https://www.quantumjitter.com/project/box/).
